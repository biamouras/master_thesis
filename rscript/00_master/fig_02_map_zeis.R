...
zeis14_sf <- sf::st_read('data/shp/zones/zeis_2016_clean.shp', quiet = T) %>% 
  dplyr::mutate(zeis = as.numeric(stringr::str_replace_all(layer, 'ZEIS\\_','')),
                id = 1:dplyr::n(),
                year = 'PDE 2014') %>% 
  dplyr::select(id, zeis, year) %>% 
  sf::st_transform(prjdegrees)
...
p <- ggplot()+
  ...
  geom_sf(data = zeis14_sf, aes(fill = paste('zeis',zeis)), color = NA) +
...
ggsave('latex/dissertacao_bms_vfull/figuras/revisao/map_zeis.png',
