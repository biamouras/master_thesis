...
knitr::opts_knit$set(root.dir = 'G:/Meu Drive/beatriz.moura@usp.br 2020-10-13 08 24/Projects/master_thesis')
...
seg_div_eq <- seq(0,max(df$seg2, na.rm=T),length.out = 4)
...
# numeric analysis for housing location
```{r}
hs_df <- housing %>%
  dplyr::left_join(df, by = c('id_grid' = 'id')) %>% 
  mutate(db = factor(db,
